...
test.group('Pagamentos e Compras', () => {
  ...
  test('deve realizar uma compra com sucesso (calculando valor no backend)', async ({ client, assert }) => {
  ...
  test('deve falhar ao acessar rota protegida sem token', async ({ client }) => {
    ...
    response.assertBodyContains({ success: false, message: 'Token de autenticação não fornecido ou inválido' })
  })
